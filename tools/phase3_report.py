...
def _analytic_validation():
    ...
    s_rot = compute_surface_strain(
        _recon(flat, flat @ rot.T - flat), ref_2d, strain_size=5, winstepsize=16
    )
    ...
    rows = [
        ...
        (
            "uniaxial plane exx",
            f"{np.nanmedian(s_pl.exx[1][interior]):.6f}",
            f"= {expected:.6f}",
            plane_err < 1e-6,
        ),
        (
            "uniaxial cylinder eyy",
            f"{np.nanmedian(s_cy.eyy[1][interior]):.6f}",
            f"~ {expected:.6f}",
            cyl_err < 0.02 * expected + 5e-5,
        ),
    ]
...
def _vsg_sensitivity():
    ...
    for ssize in sizes:
        strain = compute_surface_strain(
            _recon(flat, disp), ref_2d, strain_size=ssize, winstepsize=16
        )
...
def _realistic(workdir):
    ...
    gt_world = gt["world"]
    gt_rec = Reconstruction3D(
        gt_world,
        gt_world - gt_world[0][None],
        np.zeros(gt_world.shape[:2]),
        np.full(gt_world.shape[:2], TRACKED, np.uint8),
    )
...
def _page_summary(pdf, rows, passed):
    ...
    ax0 = fig.add_axes([0.08, 0.80, 0.84, 0.05])
    ax0.axis("off")
    ax0.text(
        0.5,
        0.5,
        "ANALYTIC GATE PASSED" if passed else "ANALYTIC GATE FAILED",
        ha="center",
        va="center",
        fontsize=18,
        fontweight="bold",
    ...
    ax = fig.add_axes([0.08, 0.50, 0.84, 0.26])
    ax.axis("off")
    ...
    t = ax.table(
        cellText=cells,
        colLabels=["test", "recovered", "expected", "result"],
        cellColours=colors,
        colWidths=[0.42, 0.22, 0.20, 0.16],
        loc="upper center",
    )
...
def _page_vsg(pdf, vsg_len, rms, sizes):
    ...
    ax.set_xlabel("VSG size (px)")
    ax.set_ylabel("RMS exx error (x1e-3 strain)")
...
def main() -> int:
    ...
    print(
        f"  VSG optimum: strain_size={sizes[int(np.argmin(rms))]} (RMS {rms.min() * 1000:.3f}e-3)"
    )
    if not passed:
