...
- 2026-09-14 v1.16.0 — **标定诊断 WP3：编码点圆心改用去背景的窗口质心（分支 `feat/coded-dot-centre`，未合并，1173 tests）**。
  作者决定：WP6 合并进本地 main（未推送），两种检查默认以琥珀色警告显示，移植 WP3。新模块
  `calibration/dot_centre.py`：编码点完成索引后，每个点先从半灰度轮廓量出椭圆（填洞，甜甜圈基准点算一个点），
  再在 1.45 倍半径的椭圆窗内按"相对背景的对比度减噪声底"加权求质心，迭代两次。背景是过 1.35–1.5 倍半径
  环带的平面（取环带中位数），噪声底为环带稳健标准差（1.4826×MAD）的 3 倍。窗口碰到其他暗像素或图像边缘的点、
  边界弧拟合点、细化失败或移动超过半个半径的点，保留原圆心。圆点阵（`findCirclesGrid`）、棋盘格和偏心校正不变。
  **与简报原型的差别（真实照片逼出来的）**：原型用环带第 90 百分位的常数背景。在 Challenge 1.0 S5 实拍
  （大圆点、光照不均）上，它把每个圆心往暗侧拉约 1 px（标定 RMS 0.59 → 0.85 px）；改成平面背景后在 S5 上仍比旧检测器差（光束平差 + 板形
  0.0869 → 0.1144 px：窗内背景像素全部带权，纸面纹理会带偏圆心：合成纹理 4 灰度下误差 0.050 px，加噪声底后 0.016 px）。
  **真值验收**（Riley 渲染，20 姿态 × 2 相机，OpenCV 4.14，复现脚本）：编码点 0.0168 → 0.0007 px（无噪声）、
  0.0171 → 0.0040（1.5 灰度）、0.0179 → 0.0078（3 灰度），基准点与普通点一致，无点被保护或失败；圆点阵不变；
  满足 WP3 验收界限。**真实照片**（检出张数和点号完全不变）：去掉板形误差后（全视图 release-object）残差
  Challenge 1.0 S1 0.0445/0.0469 → 0.0294/0.0324 px，Challenge 2.0 08 0.0763/0.1055 → 0.0313/0.0434 px；S5 无全视图，
  用光束平差 + 板形优化比较为 0.0869 → 0.0906 px（+4%），但优化后两种警告都不再出现（旧检测器左相机仍报）。
  检测耗时每张 5 Mpx 图多约 50–60 ms（真值 19 → 69 ms，Challenge 2.0 691 → 753 ms）。测试套件合成图 0.035 → 0.007 px。
...
- 2026-09-14 v1.14.0 — **标定诊断 WP0–WP2（分支 `feat/calib-diagnostics`，未合并，1126 tests）**。
  ...
  （Challenge 2.0 相机，16 组评估）表明，低重投影 RMS 背后有两种无声失效：覆盖半径之外的畸变外推
  （角点射线误差最高 2.1 px，无 PSF 时 4.0 px，RMS 仍为 0.029 px）和模型失配（约 1 px 的非径向畸变
  被 fx/cx/cy 吸收，3D 网格偏 0.70 mm、长度误差最高 347 µε，RMS 只从 0.029 升到 0.051 px）。
  **WP0**：`StereoResult.detections`（新字段，带默认值）返回最终求解实际用到的检测点（点阵靶为偏心校正后的
  圆心）；CLI 与 GUI 的 bundle 调整改用它们。E4 真值验证：圆点阵 bundle 在未校正圆心上长度误差
  −5.65 ± 0.40 µε（偏心校正被丢掉），在校正圆心上 −0.62 ± 0.58 µε；编码点阵 −4.64 → −0.96 µε。
  默认选项结果由 `tests/test_calib_pinning.py` 钉住（OpenCV 5.0 与 4.14 相差 ≤ 3.4e-6，容差 2e-5）。
  **WP1**：新模块 `calibration/diagnostics.py`（无 Qt），每台相机报告 r_cover、r_corner、cover_ratio，
  以及 k3 自由与 k3 固定两个拟合在 49×41 传感器网格上、只在 r_cover 内拟合仿射后的分歧（预检 A）；
  分歧超过 0.3 px 报 `EXTRAPOLATION_UNDETERMINED`，覆盖不足报 `LOW_COVERAGE`（info，暂定 0.8）。
  真值验收：该报的 10 台相机全报、不该报的 5 台（及无 PSF 左相机）全不报，与预检 A 最大偏差 0.0048 px。
  **WP2**：残差结构检查（预检 B）：6×5 分格平均的 excess 与三次向量场比值，两者都超过 3 报
  `LENS_MODEL_INADEQUATE`。真值验收：两台失配相机 22.94/27.76、7.57/7.72×（与预检 B 一致），
  12 台可表示相机最大 1.10/0.80×，无一误报。诊断耗时 0.05–0.09 s/次（复用求解自身的单目拟合，每台相机
  只多一次单目求解）。所有阈值为带证据注释、可覆盖的具名常量；发现带稳定代码、严重度、数值和英文消息，
  GUI 将来按代码翻译。**待办**：WP6（CLI/GUI/YAML 集成与翻译，并在默认显示前做真实数据误报检查）、
  WP3（编码点阵圆心，先在 stereo_gt 侧复现原型）、WP4（等 E1/E2 数据）、WP5（作者决定）。
- 2026-09-13 v1.13.0 — **批次 V：可用性评估全量修复（未发版，未提交，1092 tests）**。
  起因：同日的全流程可用性评估（真实数据 + 12/24 Mpx 合成序列 + 脚本驱动真实主窗口），
  结论“尚不适合交给外部新用户”：5 阻断、12 高、13 中、若干低。用户指示“全部修复”，
  **本地未推送提交 28027ce 不动**；von Mises 先只做说明，作者看过后决定改成与 2D 一致（见下）。并行工作流：主线（计算、GUI
  主体、文档）+ OPS（冻结版运维、demo、self-test）+ EXP（导出）+ VIEW（查看器提速）+ WS-D（打包、CI、
  发布文档）。
  **结果正确性**：① 逐帧掩膜按 {0,1} 读取（0/255 文件让 IC-GN 每步只走 1/255，停在整数初值旁；
  Case 3 参考帧有效点 34%→69%，上限 71%）；② 右相机有效性随位移一起插值（顶点全有效 + 边长上限
  + 最近邻限 1 步长），最终有效 = 左 ∧ 右，三种策略都改（24 Mpx 例中 37–50% 标为有效、3D 误差
  10–20 mm 的点不再输出）；③ 诚实门改为用 u_accum 梯度的仿射子集比较 + 两级邻域支持（强阈值 =
  0.6 × 门阈值，≥3 个 1 px 内强邻居），Case 3 论文参数全节点中位有效率 25%→60%（ROI 内 85%）；
  代价：f33 附近 46%→37%，已在 gate 文档里记录；④ 首帧立体链接接受阈值 ZNSSD ≤ 0.6、极线距离
  ≤ 2 px（默认开，ADVANCED > Result checks 可调）；⑤ 零填充帧只作废该帧（并行相机同样）；取消时
  保留追踪完成的全部帧（门只验证已追踪前缀）；quality 改为逐帧时序 ZNSSD；S2 用逐帧掩膜；
  标定保留全部畸变系数、DICe K4–K6/S/T、skew 一致的去畸变、图像尺寸校验；OpenCV 4.x 回退
  undistortPointsIter；有效率分母 = ROI 内节点。
  **无种子点**：自动视差先验（5 个探测子区整图匹配，极线 5 px 检验，作为 Seed.user_hint_uv 传播），
  12 Mpx 合成首帧匹配 76%→100%（零中心搜索在新接受阈值下只有 12.8%），耗时约 2 s；GUI 加
  “自动放点”和琥珀色就绪提示。
  **速度 / 内存**：L 相机验证门与 R 相机追踪重叠（6 帧 12 Mpx 52→45 s，结果与并行路径一致）；门 8 线程、
  样条系数每帧一次；立体 NCC 种子线程化 1.5→0.5 s（逐位一致）；准备阶段首条进度 0.3 s、约 5 s 开始追踪
  （原 14–34 s 无进度）；掩膜按 uint8、帧按原始位深缓存；内存预检 4.83 GiB 对实测 4.72 GiB。
  **查看器**（VIEW）：12 Mpx × 40 帧每次浏览操作的 GUI 线程最长停顿 0.25–3.2 s → 6–62 ms；渲染与大帧
  解码进工作线程（代次标记），变形模式每相机一次 Delaunay（viz3d/raster），缓存按字节限额，裂纹 3D 帧
  2.4–2.6 s → 8–16 ms；全场浏览内存增长 +1159 MB → +351 MB；首次打开 3D 仍约 0.79 s（pyvista 导入），
  现有忙碌提示。收尾：3D 导出用交互视图的相机；叠加层按节点居中（原偏 (step−1)/2 px，画布 + 应变窗 +
  导出一起改）；导出与应变窗的裂纹屏障直接用 bool ROI（去掉 12 Mpx 96 MB 的 float 副本）。
  **GUI / 导出 / 运维**：导出窗可关闭（1.0.0–1.1.0 起就关不掉，主窗也退不出）；运行中禁用新建/打开/最近并
  绑定启动时的工程；会话原子保存 + 失败弹窗；最小窗口 960×600、右栏滚动、标签宽度按字体度量；就绪检查
  内容（标定能否加载、左右是否同一批文件、尺寸、ROI 掩膜尺寸）且未就绪时禁用 Run；同目录 L/R 按 6 种
  命名自动拆分；逐帧掩膜导入；标定嵌入 .aldic3d、缺图也能打开看结果；动画失败不再报成功、GIF 用 Pillow
  流式写且帧率生效、数据导出有进度可取消、导出参数取自 run_params；日志文件 + 崩溃对话框 + faulthandler、
  后台 JIT 预热（冷 23 s，其后首跑 0.3 s）、`al-dic-3d demo` / `self-test`（12 项）、2D 目录翻译一并安装；
  命令行首个 Ctrl+C 写出已算帧（退出码 130），单一格式失败不影响其他格式，输出为 ASCII。
  **打包 / 文档**：面向用户的 CHANGELOG.md（发布说明取自它）、锁定的构建依赖、spec 去掉 TBB/UCRT、
  build-exe.yml、CI 加 ruff 与依赖上下限作业；README 快速开始改用 `al-dic-3d demo`（不再依赖 examples/，
  与 28027ce 是否推送无关），CI 表述改正；用户手册 11 章更新。
  **验证**：全量 pytest 1092 passed（-n 6）；ruff 干净；i18n 780 条 × 7 语言全部完成、扫描干净
  （extract 改为 -no-obsolete；fill 正则跳过 extracomment；gui 里的 `#:` 注释会被 lupdate 当成译者注，
  已改为 `#`）；MATLAB 对位 P1/P2 PASSED（工具修复：左帧按基线编号暂存，h5py 进 [dev]）；
  脚本驱动真实主窗口在 12 Mpx × 40 帧上重走评估时的整条流程（离屏、5 ms 心跳）：GUI 内分析
  616 s（原，有心跳；无心跳 516 s）→ 319 s，首条进度 16.2 s → 0.3 s，最长无进度 32.6 s → 6.8 s，
  运行中 >100 ms 停顿 402 → 8 次，峰值 RSS 6.75 → 5.89 GB；浏览结果最长停顿 811 → 46 ms，切场
  563 → 16 ms，切右相机 1626 → 20 ms，应变窗逐帧 983 → 18 ms，首次打开 3D 2656 → 688 ms，重新
  打开会话 4.5 → 2.3 s。
  **von Mises**：原 3D 与 MATLAB（computeStrain3D.m:51）为 √(e1²+e2²−e1e2+3R²) = √(m²+6R²)，剪切算了
  两遍（单轴高 32%、纯剪高 41%）；作者确认 MATLAB 此处有误，已改为 2D 的 √(exx²+eyy²−exx·eyy+3exy²)，
  与 al_dic 逐位一致（测试钉住），读取旧会话时由存储的分量重算。对位门禁只比 U/V/W/Z，不受影响。
  **没有改的**：28027ce。**遗留**：首次打开 3D 视图 0.79 s；GUI 比 CLI 慢 30%
  在受控负载下不能复现（6 帧 12 Mpx：CLI 52 s、GUI 56 s）。
- 2026-07-30 v1.12.0 — **批次 P4：诚实度门提速 3.3× + 全程进度可见（包版本 1.1.0，`b7da5e2`，687 tests）**。
  G4 规模实测的头号待办结案：400 帧 × 5Mpx 上 66% 墙钟时间不在追踪循环内（追踪 410+445 s，两段空档
  951+986 s），根因 = `_gate_by_znssd` 在 `run_aldic` 返回后逐帧重算 ZNSSD，**3.06 s/帧 vs 它所验证的
  追踪 1.02 s/帧**，且全程零进度。
  **提速**（真实 Tier B 数据实测 2.97→0.90 s/帧 = **3.29×**，单相机 ~16 min→~5 min，双相机 400 帧省
  ~27 min）两处独立收益：① `map_coordinates(prefilter=True)` 在**每个分块调用**里重滤整幅 5Mpx 变形图
  （6 分块 = 6 次三次样条滤波，占门时间 38%）→ 提升为每次调用滤一次、分块 `prefilter=False`；
  **构造上逐位一致**（`mode='constant'` 下 scipy 自身 `_prepad_for_spline_filter` 返回 npad=0，
  这正是它内部所做的计算）；② map_coordinates 释放 GIL（实测 4 线程 3.26×）→ 点分块走小线程池
  `min(4, cpu-2)`，P1.3 的分块预算**按 worker 均分**故峰值内存不变，分块写不相交行并按提交序重组
  → 任意 worker 数确定性（1/2/4 已断言）。**拒绝**了平移专用重采样器：在 3.3× 之上再换 ~1.3× 不值得
  改动决定节点去留的度量的浮点求和顺序（~1 ULP）。
  **逐位一致证据**：24 个真实 Tier B 帧、275,184 个值上原始 ZNSSD `max|差| = 0.000e+00`（与阈值无关）；
  判决集/n_gated/valid/u_accum 逐元素一致，含一个真实杀掉 206 个节点的阈值。
  **可见性**（压测复现的"像卡死"）：temporal_track 接管完整进度带（引擎 [0,0.6]、门 (0.6,1.0]
  "verifying frame k/N"）；**顺序 track_both 路径此前根本不转发追踪进度**（默认配置双相机全程无进度）
  → L[0,0.45)/R[0.45,0.9)/装配[0.9,1.0]，S2/S3 同洞一并接线；al-dic 0.7 的 progress_fn 非单调
  （0.99 后回落 0.95），2D 只读故加 `_Ratchet` 单调闸；停止改为**每验证一帧轮询一次**（取消最多多花
  一帧而非整趟验证），且门未触及的帧**丢弃而非发出**——"未验证却报 n_gated=0"与"已验证确实干净"外观
  相同,正是此门存在的意义。
  奇偶校验 P1+P2 仍过（P2 恰好断言门在真实 S3 数据上仍留下可用有效率，那里门确实会咬）。
