...
class ImportPage(WorkflowPage):
    def build(self) -> None:
        ...
        left_col = self._camera_column(self.tr("Left camera"), self._left_list, self._add_left)
        right_col = self._camera_column(self.tr("Right camera"), self._right_list, self._add_right)
    ...
    def _camera_column(self, title: str, lst: QListWidget, adder) -> QWidget:
        col = QWidget()
