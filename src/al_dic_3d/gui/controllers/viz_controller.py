...
Scattered nodal values become a dense image-space overlay exactly like the 2D
app: ``scatter_to_grid`` (Delaunay + CloughTocher, NaN outside the hull) builds
a regular grid over the node bounding box at ``mesh_step // 4`` resolution, a
reference-frame ROI mask (or the valid-node support fallback) knocks out pixels
outside the region of interest, and a matplotlib colormap turns the masked grid
into RGBA with NaN -> alpha 0.

3D adaptations over the 2D original:

* Node positions come from the ``CorrespondenceSet`` (``xL``/``xR`` per frame)
  and may contain NaN rows (invalid nodes) — rows are filtered before
  triangulation instead of assuming a fully finite mesh.
* When no drawn ROI mask exists — always the case for the RIGHT camera, which
  has no ROI drawing UI — the support falls back to the union of Delaunay
  triangles whose three vertices carry valid (finite) values, so both the
  outside of the point cloud and NaN-node holes stay transparent.
* Reference interpolators are cached per node set (bytes key), because one
  controller may serve LEFT and RIGHT node clouds.
...
The compute core is numpy/scipy/cv2/matplotlib only; Qt appears only at the
``QPixmap`` edge (``render_field``).
"""
...
class VizController3D:
    """Manages dense field rendering with the 2D two-level caching scheme."""

    def __init__(self) -> None:
        # Tier 1: interpolation results {(frame, field, deformed) -> (data, xg, yg, out_step)}
        self._interp_cache: dict[tuple, tuple] = {}
        # Tier 2: colored pixmaps {(frame, field, cmap, vmin, vmax, has_mask, deformed) -> QPixmap}
    ...
    def render_field_rgba(
        ...
        if cached is not None:
            ...
            # controller serves the L and R clouds without cross-talk.
            if deformed:
        ...
        if deformed and interp_key in self._warp_cache:
            ...
            if eff is not None and xg is not None:
                mask_to_use = _lookup_outside(eff, xg, yg)
